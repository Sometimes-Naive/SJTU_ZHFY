...
def xscase_info_data_process(case):
    ...
    info = case_info(File_name,'xzcase_info_data','行政案件')
    return info


def xsay_info_data_process(case):
    #读取网上搜索的相关分类案由信息，存储名为msay_key_2019.txt，输出为msay_key文件
    ay_key = xsay_key_extract('xzay_key_2019.txt', 'xzay_key')
    ay_key = json_data_r(data_path + 'xzay_key')
    ...
    json_data_w(ay_info, data_path + 'xzay_data_pre')
    json_data_w(error_case, data_path + 'xzerror_data')
    info = ay_info_process(10, 'xzay_data_pre', 'xzay_info_data','行政案由')
...
if __name__ == '__main__':
    ...
    case = AllData.objects(省份='重庆', 案件类别='行政案件')
    ...
    # print(number_info)
    File_name = 'xzpre_data'
    a = case_info(File_name,'xzcase_info_data','行政案件')
